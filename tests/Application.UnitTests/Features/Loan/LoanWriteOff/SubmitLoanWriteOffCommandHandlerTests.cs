...
public sealed class SubmitLoanWriteOffCommandHandlerTests
{
    private static SubmitLoanWriteOffCommandHandler BuildHandler(ZARI.Infrastructure.Persistence.AppDbContext dbContext, ZARI.Application.Abstractions.Identity.IPermissionService? permissions = null) =>
        new(dbContext,
            LoanTestFixtures.SuccessHandler<SubmitForApprovalCommand, Result<ApprovalRequestResponse>>(Result.Success(new ApprovalRequestResponse(Guid.NewGuid(), "LOAN_WRITE_OFF", "x", "br-1", "u1", DateTimeOffset.UtcNow, "PENDING", "SUBMIT", null, [], DateTimeOffset.UtcNow))),
            LoanTestFixtures.SuccessHandler<CreateNotificationCommand, Result<NotificationResponse>>(Result.Success(new NotificationResponse(Guid.NewGuid(), "LOAN_WRITE_OFF", "x", "br-1", "SUBMITTED", "APPROVAL_NEEDED", "msg", null, [], DateTimeOffset.UtcNow))),
    ...
    public async Task HandleAsync_Should_Submit_When_Draft()
    {
        ...
        var result = await handler.HandleAsync(new SubmitLoanWriteOffCommand(writeOff.Id, "u1"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Found()
    {
        ...
        var result = await handler.HandleAsync(new SubmitLoanWriteOffCommand(Guid.NewGuid(), "u1"), TestContext.Current.CancellationToken);
