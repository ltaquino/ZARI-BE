...
public sealed class RejectLoanDisbursementCommandHandlerTests
{
    private static RejectLoanDisbursementCommandHandler BuildHandler(ZARI.Infrastructure.Persistence.AppDbContext dbContext, ZARI.Application.Abstractions.Identity.IPermissionService? permissions = null) =>
        new(dbContext,
            LoanTestFixtures.SuccessHandler<DecideApprovalRequestCommand, Result<ApprovalRequestResponse>>(Result.Success(new ApprovalRequestResponse(Guid.NewGuid(), "LOAN_DISBURSEMENT", "x", "br-1", "u1", DateTimeOffset.UtcNow, "REJECTED", "SUBMIT", null, [], DateTimeOffset.UtcNow))),
            LoanTestFixtures.SuccessHandler<CreateNotificationCommand, Result<NotificationResponse>>(Result.Success(new NotificationResponse(Guid.NewGuid(), "LOAN_DISBURSEMENT", "x", "br-1", "REJECTED", "ACTIVITY", "msg", null, [], DateTimeOffset.UtcNow))),
            permissions ?? LoanTestFixtures.AllowAllPermissionService());

    private static async Task<(ZARI.Infrastructure.Persistence.AppDbContext db, LoanDisbursement disbursement)> Seed(string status = "PENDING_APPROVAL")
    {
    ...
    public async Task HandleAsync_Should_Reject_Back_To_Draft()
    {
        ...
        var result = await handler.HandleAsync(new RejectLoanDisbursementCommand(disbursement.Id, "approver1", "fix this"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Pending_Approval()
    {
        ...
        var result = await handler.HandleAsync(new RejectLoanDisbursementCommand(disbursement.Id, "approver1", "fix"), TestContext.Current.CancellationToken);
