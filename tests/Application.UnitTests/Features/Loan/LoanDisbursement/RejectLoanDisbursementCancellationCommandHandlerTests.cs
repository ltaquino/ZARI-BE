...
public sealed class RejectLoanDisbursementCancellationCommandHandlerTests
{
    private static RejectLoanDisbursementCancellationCommandHandler BuildHandler(ZARI.Infrastructure.Persistence.AppDbContext dbContext, ZARI.Application.Abstractions.Identity.IPermissionService? permissions = null) =>
        new(dbContext,
            LoanTestFixtures.SuccessHandler<DecideApprovalRequestCommand, Result<ApprovalRequestResponse>>(Result.Success(new ApprovalRequestResponse(Guid.NewGuid(), "LOAN_DISBURSEMENT", "x", "br-1", "u1", DateTimeOffset.UtcNow, "REJECTED", "CANCEL", null, [], DateTimeOffset.UtcNow))),
            LoanTestFixtures.SuccessHandler<CreateNotificationCommand, Result<NotificationResponse>>(Result.Success(new NotificationResponse(Guid.NewGuid(), "LOAN_DISBURSEMENT", "x", "br-1", "CANCELLATION_REJECTED", "ACTIVITY", "msg", null, [], DateTimeOffset.UtcNow))),
            permissions ?? LoanTestFixtures.AllowAllPermissionService());

    private static async Task<(ZARI.Infrastructure.Persistence.AppDbContext db, LoanDisbursement disbursement)> Seed(string status = "PENDING_CANCELLATION", bool withApprovalRequest = true)
    {
    ...
    public async Task HandleAsync_Should_Reject_And_Restore_Posted_Status()
    {
        ...
        var result = await handler.HandleAsync(new RejectLoanDisbursementCancellationCommand(disbursement.Id, "hqadmin", "not justified"), TestContext.Current.CancellationToken);
    ...
    {
        var (db, disbursement) = await Seed(status: "POSTED", withApprovalRequest: false);
        var handler = BuildHandler(db);

        var result = await handler.HandleAsync(new RejectLoanDisbursementCancellationCommand(disbursement.Id, "hqadmin", "reason"), TestContext.Current.CancellationToken);
    ...
    {
        var (db, disbursement) = await Seed(withApprovalRequest: false);
        var handler = BuildHandler(db);
