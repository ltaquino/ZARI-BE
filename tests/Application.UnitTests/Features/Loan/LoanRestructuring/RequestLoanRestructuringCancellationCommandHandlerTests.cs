...
public sealed class RequestLoanRestructuringCancellationCommandHandlerTests
{
    private static RequestLoanRestructuringCancellationCommandHandler BuildHandler(ZARI.Infrastructure.Persistence.AppDbContext dbContext, ZARI.Application.Abstractions.Identity.IPermissionService? permissions = null) =>
        new(dbContext,
            LoanTestFixtures.SuccessHandler<SubmitForApprovalCommand, Result<ApprovalRequestResponse>>(Result.Success(new ApprovalRequestResponse(Guid.NewGuid(), "LOAN_RESTRUCTURING", "x", "br-1", "u1", DateTimeOffset.UtcNow, "PENDING", "CANCEL", null, [], DateTimeOffset.UtcNow))),
            LoanTestFixtures.SuccessHandler<CreateNotificationCommand, Result<NotificationResponse>>(Result.Success(new NotificationResponse(Guid.NewGuid(), "LOAN_RESTRUCTURING", "x", "br-1", "CANCELLATION_REQUESTED", "APPROVAL_NEEDED", "msg", null, [], DateTimeOffset.UtcNow))),
            permissions ?? LoanTestFixtures.AllowAllPermissionService());

    private static async Task<(ZARI.Infrastructure.Persistence.AppDbContext db, LoanRestructuring restructuring)> Seed(string status = "POSTED", bool withPayment = false)
    {
        ...
        if (withPayment)
        {
            ...
            {
                PaymentNo = "LOAN-PMT-0001", BranchId = branch.Id, LoanAccountId = newAccount.Id, PaymentDate = DateTimeOffset.UtcNow,
    ...
    public async Task HandleAsync_Should_Request_Cancellation_When_Posted()
    {
        ...
        var result = await handler.HandleAsync(new RequestLoanRestructuringCancellationCommand(restructuring.Id, "u1", "encoding error"), TestContext.Current.CancellationToken);
    ...
    {
        var (db, restructuring) = await Seed(status: "DRAFT");
        var handler = BuildHandler(db);

        var result = await handler.HandleAsync(new RequestLoanRestructuringCancellationCommand(restructuring.Id, "u1", "reason"), TestContext.Current.CancellationToken);

        result.IsFailure.Should().BeTrue();
        result.Error!.Code.Should().Be("LoanRestructuring.NotPosted");
        await db.DisposeAsync();
    }
