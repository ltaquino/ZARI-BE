...
/// <summary>
/// The final status flip uses ExecuteUpdateAsync, so only guard clauses and the pre-decide
/// downstream re-check are exercised here — every reversal dependency is faked and never needs to
/// actually run.
/// </summary>
public sealed class ApproveStockOpnameCancellationCommandHandlerTests
{
    private static ApproveStockOpnameCancellationCommandHandler Handler(ZARI.Infrastructure.Persistence.AppDbContext db, ZARI.Application.Abstractions.Identity.IPermissionService? permissions = null) =>
        new(db,
    ...
    private static ApprovalRequestResponse Dummy() => new(Guid.NewGuid(), "STOCK_OPNAME", "x", "br-1", "admin.hq", DateTimeOffset.UtcNow, "APPROVED", "CANCEL", null, [], DateTimeOffset.UtcNow);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Found()
    {
        ...
        var result = await Handler(db).HandleAsync(new ApproveStockOpnameCancellationCommand(Guid.NewGuid(), "admin.hq", null), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Forbidden()
    {
        ...
        var result = await Handler(db, permissions).HandleAsync(new ApproveStockOpnameCancellationCommand(opname.Id, "admin.hq", null), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Pending_Cancellation()
    {
        ...
        var result = await Handler(db).HandleAsync(new ApproveStockOpnameCancellationCommand(opname.Id, "admin.hq", null), TestContext.Current.CancellationToken);
