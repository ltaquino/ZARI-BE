...
public sealed class RejectGoodsIssueCancellationCommandHandlerTests
{
    private static RejectGoodsIssueCancellationCommandHandler Handler(ZARI.Infrastructure.Persistence.AppDbContext db, ZARI.Application.Abstractions.Identity.IPermissionService? permissions = null) =>
        new(db,
    ...
    private static ApprovalRequestResponse Dummy() => new(Guid.NewGuid(), "GOODS_ISSUE", "x", "br-1", "admin.hq", DateTimeOffset.UtcNow, "REJECTED", "CANCEL", null, [], DateTimeOffset.UtcNow);
    ...
    private static async Task<(ZARI.Infrastructure.Persistence.AppDbContext db, GoodsIssue issue, Branch branch)> Seed(string status = "PENDING_CANCELLATION", bool withApprovalRequest = true)
    {
        ...
        issue.CancelReason = "wrong warehouse";
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Found()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectGoodsIssueCancellationCommand(Guid.NewGuid(), "admin.hq", "stands"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Forbidden()
    {
        ...
        var result = await Handler(db, permissions).HandleAsync(new RejectGoodsIssueCancellationCommand(issue.Id, "admin.hq", "stands"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Pending_Cancellation()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectGoodsIssueCancellationCommand(issue.Id, "admin.hq", "stands"), TestContext.Current.CancellationToken);
