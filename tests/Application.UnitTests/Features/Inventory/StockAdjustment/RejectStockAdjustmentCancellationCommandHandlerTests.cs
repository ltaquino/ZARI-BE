...
public sealed class RejectStockAdjustmentCancellationCommandHandlerTests
{
    private static RejectStockAdjustmentCancellationCommandHandler Handler(ZARI.Infrastructure.Persistence.AppDbContext db, ZARI.Application.Abstractions.Identity.IPermissionService? permissions = null) =>
        new(db,
    ...
    private static ApprovalRequestResponse Dummy() => new(Guid.NewGuid(), "STOCK_ADJUSTMENT", "x", "br-1", "admin.hq", DateTimeOffset.UtcNow, "REJECTED", "CANCEL", null, [], DateTimeOffset.UtcNow);
    ...
    private static async Task<(ZARI.Infrastructure.Persistence.AppDbContext db, StockAdjustment adjustment, Branch branch)> Seed(string status = "PENDING_CANCELLATION", bool withApprovalRequest = true)
    {
        ...
        adjustment.CancelReason = "wrong qty";
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Found()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectStockAdjustmentCancellationCommand(Guid.NewGuid(), "admin.hq", "stands"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Forbidden()
    {
        ...
        var result = await Handler(db, permissions).HandleAsync(new RejectStockAdjustmentCancellationCommand(adjustment.Id, "admin.hq", "stands"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Pending_Cancellation()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectStockAdjustmentCancellationCommand(adjustment.Id, "admin.hq", "stands"), TestContext.Current.CancellationToken);
