...
public sealed class CreateStockAdjustmentCommandHandlerTests
{
    private static CreateStockAdjustmentCommandHandler Handler(ZARI.Infrastructure.Persistence.AppDbContext db, ZARI.Application.Abstractions.Identity.IPermissionService? permissions = null) =>
        new(db,
            LoanTestFixtures.SuccessHandler<GetNextDocumentNumberCommand, Result<NextDocumentNumberResponse>>(Result.Success(new NextDocumentNumberResponse("ADJ-000001"))),
            new CreateNotificationCommandHandler(db), permissions ?? LoanTestFixtures.AllowAllPermissionService());

    private static CreateStockAdjustmentCommand Command(string branchId, Guid warehouseId, Guid itemId, Guid? costCenterId = null, decimal qtyBefore = 10, decimal qtyAfter = 15) =>
        new(branchId, warehouseId, DateTimeOffset.UtcNow, "DAMAGE", "remarks", costCenterId, "creator",
            [new StockAdjustmentLineInput(itemId, "BATCH-1", null, qtyBefore, qtyAfter, 50)]);
    ...
    public async Task HandleAsync_Should_Fail_When_Forbidden()
    {
        ...
        permissions.HasPermissionOnBranchAsync("STOCK_ADJUSTMENTS", FormAction.Create, "br-1", Arg.Any<CancellationToken>()).Returns(false);
    ...
    [Fact]
    public async Task HandleAsync_Should_Create_Adjustment_With_Computed_Variance()
    {
        await using var db = TestDbContextFactory.Create();
        var branch = LoanTestFixtures.Branch();
        db.Branches.Add(branch);
        ...
        var result = await Handler(db).HandleAsync(Command(branch.Id, warehouse.Id, item.Id, qtyBefore: 10, qtyAfter: 7), TestContext.Current.CancellationToken);

        result.IsSuccess.Should().BeTrue();
        result.Value!.Status.Should().Be("DRAFT");
        result.Value.Lines.Should().ContainSingle(l => l.VarianceQty == -3);
    }
}
