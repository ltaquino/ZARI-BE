...
public sealed class UpdateSalesReturnCommandHandlerTests
{
    private static UpdateSalesReturnCommandHandler Handler(ZARI.Infrastructure.Persistence.AppDbContext db, ZARI.Application.Abstractions.Identity.IPermissionService? permissions = null) =>
        new(db, new CreateNotificationCommandHandler(db), permissions ?? LoanTestFixtures.AllowAllPermissionService());

    private static UpdateSalesReturnCommand Command(Guid id, string branchId, Guid warehouseId, Guid customerId, Guid itemId, Guid uomId) =>
        new(id, branchId, warehouseId, customerId, null, DateTimeOffset.UtcNow, "updated", null, "encoder", [new SalesReturnLineInput(itemId, 2, uomId, 100, null, "VATABLE")]);

    private static async Task<(ZARI.Infrastructure.Persistence.AppDbContext db, string branchId, Guid warehouseId, Guid customerId, Guid itemId, Guid uomId, ZARI.Domain.Entities.SalesReturn order)> Seed(string status = "DRAFT")
    {
    ...
    public async Task HandleAsync_Should_Update_Draft_Return()
    {
        ...
        result.Value!.Lines.Should().ContainSingle(l => l.QtyReturned == 2);
    ...
    public async Task HandleAsync_Should_Fail_When_Forbidden()
    {
        ...
        permissions.HasPermissionOnBranchAsync("SALES_RETURNS", FormAction.Edit, branchId, Arg.Any<CancellationToken>()).Returns(false);
