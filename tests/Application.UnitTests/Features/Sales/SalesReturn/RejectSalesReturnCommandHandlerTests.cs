...
public sealed class RejectSalesReturnCommandHandlerTests
{
    private static RejectSalesReturnCommandHandler Handler(ZARI.Infrastructure.Persistence.AppDbContext db, ZARI.Application.Abstractions.Identity.IPermissionService? permissions = null) =>
    ...
    private static ApprovalRequestResponse Dummy() => new(Guid.NewGuid(), "SALES_RETURN", "x", "br-1", "user", DateTimeOffset.UtcNow, "REJECTED", "SUBMIT", null, [], DateTimeOffset.UtcNow);
    ...
    public async Task HandleAsync_Should_Reject_Pending_Return_Back_To_Draft()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectSalesReturnCommand(order.Id, "manager", "wrong item"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Found()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectSalesReturnCommand(Guid.NewGuid(), "manager", "x"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Forbidden()
    {
        ...
        var result = await Handler(db, permissions).HandleAsync(new RejectSalesReturnCommand(order.Id, "manager", "x"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Pending_Approval()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectSalesReturnCommand(order.Id, "manager", "x"), TestContext.Current.CancellationToken);
