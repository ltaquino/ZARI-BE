...
public sealed class RejectSalesInvoiceCommandHandlerTests
{
    private static RejectSalesInvoiceCommandHandler Handler(ZARI.Infrastructure.Persistence.AppDbContext db, ZARI.Application.Abstractions.Identity.IPermissionService? permissions = null) =>
    ...
    private static ApprovalRequestResponse Dummy() => new(Guid.NewGuid(), "SALES_INVOICE", "x", "br-1", "user", DateTimeOffset.UtcNow, "REJECTED", "SUBMIT", null, [], DateTimeOffset.UtcNow);
    ...
    public async Task HandleAsync_Should_Reject_Pending_Invoice_Back_To_Draft()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectSalesInvoiceCommand(invoice.Id, "manager", "wrong price"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Found()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectSalesInvoiceCommand(Guid.NewGuid(), "manager", "x"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Forbidden()
    {
        ...
        var result = await Handler(db, permissions).HandleAsync(new RejectSalesInvoiceCommand(invoice.Id, "manager", "x"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Pending_Approval()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectSalesInvoiceCommand(invoice.Id, "manager", "x"), TestContext.Current.CancellationToken);
