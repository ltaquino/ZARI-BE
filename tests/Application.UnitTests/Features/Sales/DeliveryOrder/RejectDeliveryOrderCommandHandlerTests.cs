...
public sealed class RejectDeliveryOrderCommandHandlerTests
{
    private static RejectDeliveryOrderCommandHandler Handler(ZARI.Infrastructure.Persistence.AppDbContext db, ZARI.Application.Abstractions.Identity.IPermissionService? permissions = null) =>
    ...
    private static ApprovalRequestResponse Dummy() => new(Guid.NewGuid(), "DELIVERY_ORDER", "x", "br-1", "user", DateTimeOffset.UtcNow, "REJECTED", "SUBMIT", null, [], DateTimeOffset.UtcNow);
    ...
    [Fact]
    public async Task HandleAsync_Should_Reject_Pending_Delivery_Back_To_Draft()
    {
        var (db, order) = await Seed();

        var result = await Handler(db).HandleAsync(new RejectDeliveryOrderCommand(order.Id, "manager", "wrong warehouse"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Found()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectDeliveryOrderCommand(Guid.NewGuid(), "manager", "x"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Forbidden()
    {
        ...
        var result = await Handler(db, permissions).HandleAsync(new RejectDeliveryOrderCommand(order.Id, "manager", "x"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Pending_Approval()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectDeliveryOrderCommand(order.Id, "manager", "x"), TestContext.Current.CancellationToken);
