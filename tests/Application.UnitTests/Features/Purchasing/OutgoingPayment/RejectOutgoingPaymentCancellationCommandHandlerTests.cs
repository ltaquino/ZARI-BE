...
public sealed class RejectOutgoingPaymentCancellationCommandHandlerTests
{
    private static RejectOutgoingPaymentCancellationCommandHandler Handler(ZARI.Infrastructure.Persistence.AppDbContext db, ZARI.Application.Abstractions.Identity.IPermissionService? permissions = null) =>
    ...
    private static ApprovalRequestResponse Dummy() => new(Guid.NewGuid(), "OUTGOING_PAYMENT", "x", "br-1", "user", DateTimeOffset.UtcNow, "REJECTED", "CANCEL", null, [], DateTimeOffset.UtcNow);

    private static async Task<(ZARI.Infrastructure.Persistence.AppDbContext db, ZARI.Domain.Entities.OutgoingPayment payment)> Seed(string status = "PENDING_CANCELLATION", bool withApprovalRequest = true)
    {
    ...
    public async Task HandleAsync_Should_Reject_Cancellation_Back_To_Posted()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectOutgoingPaymentCancellationCommand(payment.Id, "admin.hq", "not warranted"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Found()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectOutgoingPaymentCancellationCommand(Guid.NewGuid(), "admin.hq", "x"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Forbidden()
    {
        ...
        var result = await Handler(db, permissions).HandleAsync(new RejectOutgoingPaymentCancellationCommand(payment.Id, "admin.hq", "x"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Pending_Cancellation()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectOutgoingPaymentCancellationCommand(payment.Id, "admin.hq", "x"), TestContext.Current.CancellationToken);
