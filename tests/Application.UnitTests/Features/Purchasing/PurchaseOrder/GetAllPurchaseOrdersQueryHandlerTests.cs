...
public sealed class GetAllPurchaseOrdersQueryHandlerTests
{
    [Fact]
    public async Task HandleAsync_Should_Return_All_Orders()
    {
        ...
        await db.SaveChangesAsync(TestContext.Current.CancellationToken);
        var handler = new GetAllPurchaseOrdersQueryHandler(db, LoanTestFixtures.AllowAllPermissionService());

        var result = await handler.HandleAsync(new GetAllPurchaseOrdersQuery(), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Forbidden()
    {
        ...
        var handler = new GetAllPurchaseOrdersQueryHandler(db, permissions);
