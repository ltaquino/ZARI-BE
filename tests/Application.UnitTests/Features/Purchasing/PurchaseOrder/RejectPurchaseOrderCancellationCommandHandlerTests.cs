...
public sealed class RejectPurchaseOrderCancellationCommandHandlerTests
{
    private static RejectPurchaseOrderCancellationCommandHandler Handler(ZARI.Infrastructure.Persistence.AppDbContext db, ZARI.Application.Abstractions.Identity.IPermissionService? permissions = null) =>
    ...
    private static ApprovalRequestResponse Dummy() => new(Guid.NewGuid(), "PURCHASE_ORDER", "x", "br-1", "user", DateTimeOffset.UtcNow, "REJECTED", "CANCEL", null, [], DateTimeOffset.UtcNow);

    private static async Task<(ZARI.Infrastructure.Persistence.AppDbContext db, PurchaseOrder order)> Seed(string status = "PENDING_CANCELLATION", bool withApprovalRequest = true)
    {
    ...
    public async Task HandleAsync_Should_Reject_Cancellation_Back_To_Posted()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectPurchaseOrderCancellationCommand(order.Id, "admin.hq", "not warranted"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Found()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectPurchaseOrderCancellationCommand(Guid.NewGuid(), "admin.hq", "x"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Forbidden()
    {
        ...
        var result = await Handler(db, permissions).HandleAsync(new RejectPurchaseOrderCancellationCommand(order.Id, "admin.hq", "x"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Pending_Cancellation()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectPurchaseOrderCancellationCommand(order.Id, "admin.hq", "x"), TestContext.Current.CancellationToken);
