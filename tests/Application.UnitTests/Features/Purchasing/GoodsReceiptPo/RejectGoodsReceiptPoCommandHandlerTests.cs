...
public sealed class RejectGoodsReceiptPoCommandHandlerTests
{
    private static RejectGoodsReceiptPoCommandHandler Handler(ZARI.Infrastructure.Persistence.AppDbContext db, ZARI.Application.Abstractions.Identity.IPermissionService? permissions = null) =>
    ...
    private static ApprovalRequestResponse Dummy() => new(Guid.NewGuid(), "GOODS_RECEIPT_PO", "x", "br-1", "user", DateTimeOffset.UtcNow, "REJECTED", "SUBMIT", null, [], DateTimeOffset.UtcNow);
    ...
    [Fact]
    public async Task HandleAsync_Should_Reject_Pending_Receipt_Back_To_Draft()
    {
        var (db, receipt) = await Seed();

        var result = await Handler(db).HandleAsync(new RejectGoodsReceiptPoCommand(receipt.Id, "manager", "missing details"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Found()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectGoodsReceiptPoCommand(Guid.NewGuid(), "manager", "x"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Forbidden()
    {
        ...
        var result = await Handler(db, permissions).HandleAsync(new RejectGoodsReceiptPoCommand(receipt.Id, "manager", "x"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Pending_Approval()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectGoodsReceiptPoCommand(receipt.Id, "manager", "x"), TestContext.Current.CancellationToken);
