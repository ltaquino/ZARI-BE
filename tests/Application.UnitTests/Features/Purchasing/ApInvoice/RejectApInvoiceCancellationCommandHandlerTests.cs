...
public sealed class RejectApInvoiceCancellationCommandHandlerTests
{
    private static RejectApInvoiceCancellationCommandHandler Handler(ZARI.Infrastructure.Persistence.AppDbContext db, ZARI.Application.Abstractions.Identity.IPermissionService? permissions = null) =>
    ...
    private static ApprovalRequestResponse Dummy() => new(Guid.NewGuid(), "AP_INVOICE", "x", "br-1", "user", DateTimeOffset.UtcNow, "REJECTED", "CANCEL", null, [], DateTimeOffset.UtcNow);

    private static async Task<(ZARI.Infrastructure.Persistence.AppDbContext db, ZARI.Domain.Entities.ApInvoice invoice)> Seed(string status = "PENDING_CANCELLATION", bool withApprovalRequest = true)
    {
    ...
    public async Task HandleAsync_Should_Reject_Cancellation_Back_To_Posted()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectApInvoiceCancellationCommand(invoice.Id, "admin.hq", "not warranted"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Found()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectApInvoiceCancellationCommand(Guid.NewGuid(), "admin.hq", "x"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Forbidden()
    {
        ...
        var result = await Handler(db, permissions).HandleAsync(new RejectApInvoiceCancellationCommand(invoice.Id, "admin.hq", "x"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Pending_Cancellation()
    {
        ...
        var result = await Handler(db).HandleAsync(new RejectApInvoiceCancellationCommand(invoice.Id, "admin.hq", "x"), TestContext.Current.CancellationToken);
