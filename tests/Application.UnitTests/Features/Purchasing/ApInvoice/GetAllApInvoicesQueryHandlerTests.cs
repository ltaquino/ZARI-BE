...
public sealed class GetAllApInvoicesQueryHandlerTests
{
    [Fact]
    public async Task HandleAsync_Should_Return_All_Invoices()
    {
        ...
        await db.SaveChangesAsync(TestContext.Current.CancellationToken);
        var handler = new GetAllApInvoicesQueryHandler(db, LoanTestFixtures.AllowAllPermissionService());

        var result = await handler.HandleAsync(new GetAllApInvoicesQuery(), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Forbidden()
    {
        ...
        var handler = new GetAllApInvoicesQueryHandler(db, permissions);
