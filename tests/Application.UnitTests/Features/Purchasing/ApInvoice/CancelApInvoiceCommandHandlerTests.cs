...
public sealed class CancelApInvoiceCommandHandlerTests
{
    private static CancelApInvoiceCommandHandler Handler(ZARI.Infrastructure.Persistence.AppDbContext db, ZARI.Application.Abstractions.Identity.IPermissionService? permissions = null) =>
    ...
    public async Task HandleAsync_Should_Cancel_Draft_Invoice()
    {
        ...
        var result = await Handler(db).HandleAsync(new CancelApInvoiceCommand(invoice.Id, "encoder", "no longer needed"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Not_Found()
    {
        ...
        var result = await Handler(db).HandleAsync(new CancelApInvoiceCommand(Guid.NewGuid(), "encoder", "x"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Forbidden()
    {
        ...
        var result = await Handler(db, permissions).HandleAsync(new CancelApInvoiceCommand(invoice.Id, "encoder", "x"), TestContext.Current.CancellationToken);
    ...
    public async Task HandleAsync_Should_Fail_When_Already_Cancelled()
    {
        ...
        var result = await Handler(db).HandleAsync(new CancelApInvoiceCommand(invoice.Id, "encoder", "x"), TestContext.Current.CancellationToken);

        result.IsFailure.Should().BeTrue();
        result.Error!.Code.Should().Be("ApInvoice.AlreadyCancelled");
        await db.DisposeAsync();
    }
    ...
    public async Task HandleAsync_Should_Fail_When_Posted_Requires_Cancellation_Request()
    {
        ...
        result.IsFailure.Should().BeTrue();
        result.Error!.Code.Should().Be("ApInvoice.RequiresCancellationRequest");
        await db.DisposeAsync();
    }
}
