...
/// <summary>
/// PENDING_APPROVAL -> APPROVED. No stock/GL side effects at all — this document is pure workflow
/// state, same shape as ApprovePurchaseOrderCommandHandler. The eventual Purchase Order is what
/// actually commits to a vendor.
/// </summary>
public sealed class ApprovePurchaseRequestCommandHandler(
    ...
    : ICommandHandler<ApprovePurchaseRequestCommand, Result<PurchaseRequestResponse>>
{
    public async Task<Result<PurchaseRequestResponse>> HandleAsync(ApprovePurchaseRequestCommand command, CancellationToken cancellationToken = default)
    {
        ...
        if (!await permissionService.HasPermissionOnBranchAsync("PURCHASE_REQUESTS", FormAction.Approve, request.BranchId, cancellationToken))
            return Result.Failure<PurchaseRequestResponse>(Error.Forbidden("PurchaseRequest.Forbidden", "You do not have permission to approve this purchase request for this branch."));

        if (request.Status != "PENDING_APPROVAL")
            return Result.Failure<PurchaseRequestResponse>(Error.Validation("PurchaseRequest.NotPendingApproval", "Only purchase requests pending approval can be approved."));
        ...
        var notifyResult = await createNotificationHandler.HandleAsync(
            new CreateNotificationCommand("PURCHASE_REQUEST", request.Id.ToString(), request.BranchId, "APPROVED", "ACTIVITY",
                "approved this purchase request", command.ApproverUserId),
            cancellationToken);
