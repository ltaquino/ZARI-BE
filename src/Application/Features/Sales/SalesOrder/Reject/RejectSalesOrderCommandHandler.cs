...
public sealed class RejectSalesOrderCommandHandler(
    ...
    : ICommandHandler<RejectSalesOrderCommand, Result<SalesOrderResponse>>
{
    public async Task<Result<SalesOrderResponse>> HandleAsync(RejectSalesOrderCommand command, CancellationToken cancellationToken = default)
    {
        ...
        if (!await permissionService.HasPermissionOnBranchAsync("SALES_ORDERS", FormAction.Approve, order.BranchId, cancellationToken))
            return Result.Failure<SalesOrderResponse>(Error.Forbidden("SalesOrder.Forbidden", "You do not have permission to reject sales orders for this branch."));

        if (order.Status != "PENDING_APPROVAL")
            return Result.Failure<SalesOrderResponse>(Error.Validation("SalesOrder.NotPendingApproval", "Only sales orders pending approval can be rejected."));
        ...
        var notifyResult = await createNotificationHandler.HandleAsync(
            new CreateNotificationCommand("SALES_ORDER", order.Id.ToString(), order.BranchId, "REJECTED", "ACTIVITY",
                $"rejected this sales order — \"{command.Comments}\"", command.ApproverUserId),
            cancellationToken);
