...
public sealed class RejectSalesReturnCancellationCommandHandler(
    IAppDbContext dbContext,
    ICommandHandler<DecideApprovalRequestCommand, Result<ApprovalRequestResponse>> decideHandler,
    ICommandHandler<CreateNotificationCommand, Result<NotificationResponse>> createNotificationHandler,
    IPermissionService permissionService)
    : ICommandHandler<RejectSalesReturnCancellationCommand, Result<SalesReturnResponse>>
{
    public async Task<Result<SalesReturnResponse>> HandleAsync(RejectSalesReturnCancellationCommand command, CancellationToken cancellationToken = default)
    {
        ...
        if (salesReturn.Status != "PENDING_CANCELLATION")
            return Result.Failure<SalesReturnResponse>(Error.Validation("SalesReturn.NotPendingCancellation", "Only a sales return pending cancellation can have that request rejected."));
        ...
        salesReturn.CancelReason = null;
        await dbContext.SaveChangesAsync(cancellationToken);

        var notifyResult = await createNotificationHandler.HandleAsync(
            new CreateNotificationCommand("SALES_RETURN", salesReturn.Id.ToString(), salesReturn.BranchId, "CANCELLATION_REJECTED", "ACTIVITY",
