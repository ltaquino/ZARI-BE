...
public sealed class DeleteUomCommandHandler(IAppDbContext dbContext, IPermissionService permissionService) : ICommandHandler<DeleteUomCommand>
{
    public async Task<Result> HandleAsync(DeleteUomCommand command, CancellationToken cancellationToken = default)
    {
        ...
        if (!await permissionService.HasPermissionAsync("UOMS", FormAction.Delete, cancellationToken))
            return Result.Failure(Error.Forbidden("Uom.Forbidden", "You do not have permission to delete UOMs."));
