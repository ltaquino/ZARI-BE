...
public sealed class CreateLoanRestructuringValidator : AbstractValidator<CreateLoanRestructuringCommand>
{
    private static readonly string[] ValidFrequencies = ["MONTHLY", "SEMI_MONTHLY", "WEEKLY"];

    public CreateLoanRestructuringValidator()
    {
        RuleFor(x => x.BranchId).NotEmpty().MaximumLength(25);
        RuleFor(x => x.OldLoanAccountId).NotEmpty();
