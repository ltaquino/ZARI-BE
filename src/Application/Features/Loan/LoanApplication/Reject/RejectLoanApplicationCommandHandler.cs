...
public sealed class RejectLoanApplicationCommandHandler(
    ...
    : ICommandHandler<RejectLoanApplicationCommand, Result<LoanApplicationResponse>>
{
    public async Task<Result<LoanApplicationResponse>> HandleAsync(RejectLoanApplicationCommand command, CancellationToken cancellationToken = default)
    {
        ...
        if (!await permissionService.HasPermissionOnBranchAsync("LOAN_APPLICATIONS", FormAction.Approve, application.BranchId, cancellationToken))
            return Result.Failure<LoanApplicationResponse>(Error.Forbidden("LoanApplication.Forbidden", "You do not have permission to reject this loan application for this branch."));

        if (application.Status != "PENDING_APPROVAL")
            return Result.Failure<LoanApplicationResponse>(Error.Validation("LoanApplication.NotPendingApproval", "Only loan applications pending approval can be rejected."));
        ...
        var notifyResult = await createNotificationHandler.HandleAsync(
            new CreateNotificationCommand("LOAN_APPLICATION", application.Id.ToString(), application.BranchId, "REJECTED", "ACTIVITY",
                $"rejected this loan application — \"{command.Comments}\"", command.ApproverUserId),
            cancellationToken);
